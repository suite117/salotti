function getList(item, $list, optionals) {
  ...
  if (item) {
	...
	var field_name = optionals.label;

	if (item[field_name]) {
	  // $li.append($('<a href="#">' + item[field_name] + '</a>'));
	  $li.append($('<div class="dd-handle">' + item[field_name] + '</div>'));
	}
...
$.fn.bootnestable = function(arg1) {
  ...
  if (jQuery.isPlainObject(arg1)) {
	...
	var $bt1 = $(
		'<button type="button" data-action="expandAll" class="btn btn-default">Espandi tutto</button>')
		.data("target-id", this.attr("id"));
	var $bt2 = $(
		'<button type="button" data-action="collapseAll" class="btn btn-default">Comprimi tutto</button>')
		.data("target-id", this.attr("id"));
