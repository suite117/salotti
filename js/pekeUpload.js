		...
		.each(function() {
		  ...
		  if (options.theme == "bootstrap") {
			var html = '<a href="javascript:void(0)" class="btn btn-primary btn-file"> <span class="icon-upload icon-white"></span> '
				+ options.btnText + '</a><div class="help-block"></div>';
		  }
		  ...
		  obj.change(function() {
			...
			$("#" + options.input_text_id).val(file.name);
			  ...
			  if (options.onSubmit == false) {
				...
				if (options.type == "image") {
				  ...
				  if (options.multi == true) {
					...
					if ($imgs.length != 0)
					  $imgs.removeClass('img-responsive').width(100);

					$("#" + options.preview_id).prepend(img);
				  } else
					$("#" + options.preview_id).html(img);
				}
			  } else {
				/*
				 * obj.next('a').next('div').prepend( '<input type="text"
				 * readonly class="form-control" value="' + file.name + '">');
				 */
	  ...
	  $
		  .ajax({
			url : options.base_url + options.url
				+ (options.relative_url != null ? '?path=' + options.relative_url : ''),
			type : 'POST',
			data : formData,
			// dataType: 'json',
	...
	function progressHandlingFunction(e) {
	  if (e.lengthComputable) {
		...
		if (options.showPercent == true) {
		  ...
		  obj.next('a').next('div').find('.file').first().find('.pekeup-progress-bar:first').width(
			  percent + '%');
		}
		obj.next('a').next('div').find('.file').first().find('.pekeup-progress-bar:first').html(
